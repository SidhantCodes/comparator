...
export function ComparisonTable({ products, categories, onRemoveProduct }: ComparisonTableProps) {
  ...
  return (
    <div className="bg-white border rounded-xl">
      ...
      <div className="sticky top-16 z-20 bg-white border-b border-gray-200 shadow-sm overflow-hidden">
        <div 
          className="overflow-x-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-emerald-500 hover:scrollbar-thumb-emerald-600"
          style={{
            scrollbarWidth: 'thin',  // For Firefox
            scrollbarColor: '#10b981 #f3f4f6'  // thumb and track colors for Firefox
          }}
          ref={headerScrollRef}
          ...
          <div className="grid bg-white" style={{ gridTemplateColumns }}>
            ...
            {products.map((product) => (
              ...
              >
                ...
                {/* Product Image */}
                <div className="mb-3" onClick={() => navigate(`/product/${product.id}`)}>
                  <ImageWithFallback
                    src={product.image || ""}
                    alt={product.name}
                    className="w-20 h-24 mx-auto object-contain cursor-pointer"
                  />
                </div>
